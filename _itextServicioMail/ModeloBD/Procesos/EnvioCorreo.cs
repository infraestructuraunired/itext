            ...
            if (Thread.CurrentThread.Name != null)
            {
                ...
                try
                {
                    ...
                    else if (cnfserv.esComprobante)
                    {
                        ...
                        try
                        {
                            llaveAbreLink = ConfigurationSettings.AppSettings["Ambiente"].ToString(CultureInfo.InvariantCulture) +
                                "/correo?u=" + CryptoProviderManager.Providers["AESRandom"].Encriptar(mailingRow.mail_llave_externa + "|" + mailingRow.id_tipo_mail + "|" + mailingRow.id_mail);
                            Logger.Info("Se reemplaza link --> idmail:"+idMail+" /// link -->"+llaveAbreLink);
                        }
                        catch (Exception exEnc)
                        {
                            Logger.Info("error al asociar link : " + idMail+" //// mensaje :"+exEnc.InnerException);
                            llaveAbreLink = "#";
                        }

                        html = html.Replace("#linkAbrirCorreo", llaveAbreLink);
                        mail.Body = html;
                        mail.From = new MailAddress(cnfserv.MailRemitente);
                    }
